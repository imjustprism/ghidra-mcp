...
| `list_functions` | functions (with_address=true fn+addr, false names-only incl auto) |
...
| `extract_api_call_sequences` | ordered API-call trace of a function (behavioral) |
| `find_format_string_vulns` | printf-family calls with a non-constant format (CWE-134) |
...
<details>
<summary><b>Xrefs / CFG</b> (19)</summary>

| tool | purpose |
| --- | --- |
| `xrefs` | references for a target (direction=both fn-by-name IAT-resolved, to, from) |
| `list_callers` | direct callers |
| `list_callees` | direct callees |
| `basic_blocks` | CFG blocks |
| `function_string_refs` | strings referenced |
| `callgraph` | call graph (format=mermaid|dot, callees/callers/both, bounded) |
