...
.EXAMPLE
  ...
  .\deploy.ps1 -KillProcesses client,game  # optional extra processes to stop
#>
param(
    [string]$GhidraHome = $(
        if ($env:GHIDRA_HOME) { $env:GHIDRA_HOME }
        elseif (Test-Path "$env:USERPROFILE\scoop\apps\ghidra\current") {
            "$env:USERPROFILE\scoop\apps\ghidra\current"
        }
        else { "" }
    ),
    [switch]$SkipRust,
...
function Yeet-Locks {
    $report = @{ mcp = 0; ghidra = 0; extra = 0; files = 0 }
    ...
    if ($KillProcesses -and $KillProcesses.Count -gt 0) {
        $report.extra = Stop-NamedProcesses $KillProcesses
    }

    # brief settle so handles drop
    if (($report.mcp + $report.ghidra + $report.extra) -gt 0) {
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($GhidraHome)) {
        throw "Ghidra home not set. Pass -GhidraHome PATH or set env GHIDRA_HOME (or install via scoop: ghidra)."
    }
